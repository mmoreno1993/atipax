                ...
                <div class="modal-body">
                    ...
                    <div class="col-md-6 col-lg-6 col-sm-12 col-xs-12">
						<div class="form-group">
                            ...
                            <input maxlength="255" onkeypress="return fn_apos(event)" class="form-control" type="file" id="txtImagen" name="txtImagen" />
                        </div>
                    </div>
                    <div class="col-md-6 col-lg-6 col-sm-12 col-xs-12">
						<div class="form-group">
                            <label for="txtCatalogo">Catalogo(*):</label>
                            <input maxlength="255" onkeypress="return fn_apos(event)" class="form-control" type="file" id="txtCatalogo" name="txtCatalogo" />
                        </div>
                    </div>
                    <div class="col-md-6 col-lg-6 col-sm-12 col-xs-12">
                        <div class="form-group">
...
<script type="text/javascript">
	...
	function consultar(id)
	{
    	...
    	$.ajax({
	        type: "POST",
	        url: "<?=base_url('/admin/catalogos/obtener');?>",
	        async: true,
	        dataType: "json",
	        data: {
	            id: id
	...
	function cambiar_estado(id,estado)
	{
        ...
        $.ajax({
            type: "POST",
            async: true,
            url: "<?=base_url('/admin/catalogos/cambiar_estado');?>",
            data: {
                id: id,
                estado: estado
            },
	...
	function guardar()
	{
	    ...
	    var catalogo = $("#txtCatalogo")[0].files[0];
		...
		if($("#txtCatalogo")[0].files.length != 0)
			formData.append("catalogo", catalogo,catalogo.name);
	    ...
	    if(id == 0)
	    {
		    ...
		    if($("#txtCatalogo")[0].files.length == 0)
		    {
	            swal(
	                'Falta completar',
	                'El campo catalogo es obligatorio',
	                'warning'
	            )
	            return false;
		    }
	    ...
	    if(id == "0")
	    {
	    	$.ajax({
		        type: "POST",
		        url: "<?=base_url('/admin/catalogos/agregar');?>",
		        async: true,
		        ...
		        success: function(data){
		            ...
		            if(data == 1)
		            {
		                swal({
		                    title: 'Correcto',
		                    text: 'Se agrego el catalogo nuevo',
		                    icon: 'success',
		                    timer: 1500,
		                    buttons: false
		                    }).then(
		                    function () {
		                        window.location.href = "<?=base_url('/admin/catalogos');?>";
		                    },
		                    function (dismiss) {
		                        if (dismiss === 'timer') {
		                            window.location.href = "<?=base_url('/admin/catalogos');?>";
	    	...
	    	$.ajax({
		        type: "POST",
		        url: "<?=base_url('/admin/catalogos/modificar');?>",
		        async: true,
                enctype: 'multipart/form-data',
                mimeType: 'multipart/form-data',
                processData:false,
