...
<script type="text/javascript">
	...
	function consultar(id)
	{
    	...
    	$.ajax({
	        type: "POST",
	        url: "<?=base_url('/admin/flyers/obtener');?>",
	        async: true,
	        dataType: "json",
	        data: {
	            id: id
	...
	function cambiar_estado(id,estado)
	{
        ...
        $.ajax({
            type: "POST",
            async: true,
            url: "<?=base_url('/admin/flyers/cambiar_estado');?>",
            data: {
                id: id,
                estado: estado
            },
	...
	function guardar()
	{
	    ...
	    if(id == "0")
	    {
	    	$.ajax({
		        type: "POST",
		        url: "<?=base_url('/admin/flyers/agregar');?>",
		        async: true,
		        ...
		        success: function(data){
		            ...
		            if(data == 1)
		            {
		                swal({
		                    title: 'Correcto',
		                    text: 'Se agrego el flyer nuevo',
		                    icon: 'success',
		                    timer: 1500,
		                    buttons: false
		                    }).then(
		                    function () {
		                        window.location.href = "<?=base_url('/admin/flyers');?>";
		                    },
		                    function (dismiss) {
		                        if (dismiss === 'timer') {
		                            window.location.href = "<?=base_url('/admin/flyers');?>";
	    	...
	    	$.ajax({
		        type: "POST",
		        url: "<?=base_url('/admin/flyers/modificar');?>",
		        async: true,
                enctype: 'multipart/form-data',
                mimeType: 'multipart/form-data',
                processData:false,
